...
본 프로젝트는 시스템의 부하를 분산하고 조작 편의성을 높이기 위해, 로봇 직접 제어부(Device 1)와 원격 명령/UI 관리부(Device 2)로 나뉜 **멀티 디바이스 분산 아키텍처**를 채택했습니다. 

#### 📍 주요 구성 요소 (System Components)

**1. Device 1: 로봇 직접 제어 및 상태 퍼블리셔 (Robot Control PC)**
실제 Doosan M0609 로봇과 통신하며 물리적인 동작을 수행하는 핵심 장비입니다.
* **Robot Status Publisher:** 로봇의 실시간 상태(위치, 조인트 각도, 힘 등)를 읽어와 ROS2 `Topic`(분홍색 선)으로 발행합니다.
* **Controller Node & DSR Node:** Device 2로부터 모션 제어 명령을 `Service`(파란색 선) 형태로 수신하여, 내부 태스크 스레드를 통해 `DSR2 Controller`에 로봇 구동 명령을 하달합니다.

**2. Device 2: 원격 태스크 관리 및 Web UI (Task & UI PC)**
사용자 입력을 처리하고 전체적인 작업 프로세스(Task)를 조율하는 외부 장비입니다.
* **Task Controller:** 사용자의 명령(Web UI)을 받아 Device 1으로 적절한 `Service` 요청을 보내며, Device 1에서 퍼블리시하는 상태 `Topic`을 구독하여 모션을 모니터링합니다.
* **UI Bridge (rosbridge websocket):** ROS2 통신 환경과 로컬 웹 환경을 연결하는 브리지 노드로, 웹 브라우저 기반의 조작을 가능하게 합니다.
* **Web UI (Localhost):** 관리자가 로봇 시스템을 직관적으로 제어하고 상태를 확인할 수 있는 인터페이스입니다.

**3. 클라우드 연동 (Cloud Integration)**
* **Firebase:** 시스템의 주요 상태(Status)와 이벤트(Event) 로그는 Firebase로 전송되어, 원격 모니터링 및 데이터 관리를 지원합니다.

#### 🔄 주요 통신 방식 통신 (ROS2 Communication)
설계도에 명시된 바와 같이 두 가지 핵심 ROS2 통신 프로토콜을 사용합니다.
...
본 시스템은 두산 로봇 드라이버를 통해 실제 로봇과 연결된 상태에서 작동합니다. 실행을 위해 두 개의 터미널에서 동시에 입력한다.
...
### Step 2: 작업 노드 실행 (Terminal 2)
로봇 통신 및 RViz 실행이 완료되면, **새로운 터미널**을 열고 실행하려는 파이썬 파일 이름을 `ros2 run cobot1` 명령어 뒤에 넣어 실행합니다.

```bash
ros2 run cobot1 [실행할_파이썬_파일명]
